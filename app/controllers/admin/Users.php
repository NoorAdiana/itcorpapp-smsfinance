        ...
        if ($this->session->userdata('username') === '' || array_key_exists('username', $this->session->all_userdata()) === false) {
            ...
            if ($action == 'detail_user') {
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $data['pesan'] = 'Mohon untuk dicek kembali data yang diinput';
                    $this->load->view('admin/user_management/detail_user', $data);
                } elseif ($change_password === 1 && (trim($password) == '' || strlen($password) < 4)) {
                    $data['status'] = 'Failed';
                    $data['pesan'] = 'Password minimal 4 karekater!';
                    $this->load->view('admin/user_management/detail_user', $data);
                } else {
                    $get_secret_key = $this->users_models->get_secret_key($username);
                    $save_password = sha1(md5($password) . $get_secret_key[0]['SecretKey']);
